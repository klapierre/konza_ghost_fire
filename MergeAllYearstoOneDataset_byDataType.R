...
DPM2015<-read.csv("~/Dropbox/Ghost Fire/Data/GhostFire2015_Data/DiscPasture/GhostFire_Disc Pasture_2015.csv")%>%
  select(-disc)%>%
  gather(quad, height, Quad.A:Quad.D)%>%
  group_by(Year, Wateshed, Plot, Rep) %>% 
  summarise(meanheight=mean(height, na.rm=T))%>%
  filter(Year!="NA")
DPM2016<-read.csv("~/Dropbox/Ghost Fire/Data/GhostFire2016_Data/DiscPasture/GhostFire_Disc Pasture_2016.csv")%>%
  select(-disc)%>%
  gather(quad, height, Quad.A:Quad.D)%>%
  group_by(Year, Wateshed, Plot, Rep) %>% 
  summarise(meanheight=mean(height, na.rm=T))%>%
  filter(Year!="NA")
DPM2018<-read.csv("~/Dropbox/Ghost Fire/Data/GhostFire2018_Data/DiscPasture/GhostFire_Disc Pasture_2018.csv")%>%
  gather(quad, height, Quad.A:Quad.D)%>%
  group_by(Year, Wateshed, Plot, Rep) %>% 
  summarise(meanheight=mean(height, na.rm=T))%>%
  filter(Year!="NA")
...
Light2014Early<-read.csv("~/Dropbox/Ghost Fire/Data/GhostFire2014_Data/Light/GhostFire_Light_May2014.csv")%>%
...
Light2014Late<-read.csv("~/Dropbox/Ghost Fire/Data/GhostFire2014_Data/Light/GhostFire_Light_August2014.csv")%>%
  select(-B_average, -Light)%>%
  filter(Block!="x") %>% 
  gather(Rep, Below, Below.1:Below.2)%>%
  group_by(Year, BurnFreq, Watershed, Block, Plot, Above) %>% 
...
Light2015<-read.csv("~/Dropbox/Ghost Fire/Data/GhostFire2015_Data/Light/GhostFire_Light_June2015.csv")%>%
  select(-B_Average, -Light)%>%
  filter(Block!="x") %>% 
  gather(Rep, Below, Below.1:Below.2)%>%
  group_by(Year, BurnFreq, Watershed, Block, Plot, Above) %>% 
...
Light2016<-read.csv("~/Dropbox/Ghost Fire/Data/GhostFire2016_Data/Light/GhostFire_Light_June2016.csv")%>%
...
Light2017<-read.csv("~/Dropbox/Ghost Fire/Data/GhostFire2017_Data/Light/GhostFire_Light_May2017.csv")%>%
  select(-B_Average, -Light, -AboveLitter_Average)%>%
  group_by(Year, BurnFreq, Watershed, Block, Plot, Above) %>% 
  mutate(meanBelowLitter=mean(c(Below.1, Below.2), na.rm=T), meanBelow=mean(c(Above.Litter.1, Above.Litter.2), na.rm=T))%>%
  select(-Below.1, -Below.2, -Above.Litter.1, -Above.Litter.2) %>% 
...
Light2018<-read.csv("~/Dropbox/Ghost Fire/Data/GhostFire2018_Data/Light/GhostFire_Light_June2018.csv")%>%
  group_by(Year, BurnFreq, Watershed, Block, Plot, Above) %>% 
  mutate(meanBelowLitter=mean(c(Below.1, Below.2), na.rm=T), meanBelow=mean(c(Above.Litter.1, Above.Litter.2), na.rm=T))%>%
  select(-Below.1, -Below.2, -Above.Litter.1, -Above.Litter.2) %>% 
  mutate(LitterCanopyEffect=((meanBelowLitter/Above)*100), CanopyEffect=((meanBelow/Above)*100))%>%
...
Light_AllYears<-bind_rows(Light2014Early, Light2014Late, Light2015, Light2016, Light2017, Light2018)
write.csv(Light_AllYears, "Light_2014-2018.csv", row.names = F)
...
SC2014<-read.csv("~/Dropbox/Ghost Fire/Data/GhostFire2014_Data/Species Comp/GhostFire_SpComp_2014.csv")%>%
  select(-Experiment, -Site) %>% 
  group_by(Year, Burn.Trt, Block, Plot, spnum, Species) %>% 
  gather(Season, cover, June:August)%>%
  filter(cover!=0)

SC2015<-read.csv("~/Dropbox/Ghost Fire/Data/GhostFire2015_Data/SpeciesComp/GhostFire_SpComp_2015.csv")%>%
  select(-Experiment, -Site, -Entry, -Max.cov, -X.1, -X.2, -X.3, -X.4, -X.5, -X) %>% 
  group_by(Year, Burn.Trt, Block, Plot, spnum, Species) %>% 
  gather(Season, cover, June:August)%>%
  filter(cover!=0)

SC2016<-read.csv("~/Dropbox/Ghost Fire/Data/GhostFire2016_Data/SpeciesComp/GhostFire_SpComp_2016.csv")%>%
  select(-Experiment, -Site, -Entry, -Max.cov, -X.1, -X.2, -X.3, -X.4, -X.5, -X) %>% 
  group_by(Year, Burn.Trt, Block, Plot, spnum, Species) %>% 
  gather(Season, cover, June:August)%>%
  filter(cover!=0)

SC2017<-read.csv("~/Dropbox/Ghost Fire/Data/GhostFire2017_Data/SpeciesComp/GhostFire_SpComp_2017.csv")%>%
  select(-Experiment, -Site, -Entry, -Max.cov, -X.1, -X.2, -X, -Comments) %>% 
  group_by(Year, Burn.Trt, Block, Plot, spnum, Species) %>% 
  gather(Season, cover, June:August)%>%
  filter(cover!=0)

SC2018<-read.csv("~/Dropbox/Ghost Fire/Data/GhostFire2018_Data/SpeciesComp/GhostFire_SpComp_2018.csv")%>%
...
SD2014<-read.csv("~/Dropbox/Ghost Fire/Data/GhostFire2014_Data/Stem Density/GhostFire_SpringStemD_2014.csv")%>%
  group_by(Year, Burn, Watershed, Block, spnum, Species) %>% 
  gather(Plot, stems, p1:p6)%>%
  filter(stems!=0)%>% 
  filter(Year!="NA")
SD2015<-read.csv("~/Dropbox/Ghost Fire/Data/GhostFire2015_Data/StemDensity/GhostFire_SpringStemD_2015.csv")%>%
  select(-Format.ID) %>% 
  group_by(Year, Burn, Watershed, Block, spnum, Species) %>% 
  gather(Plot, stems, p1:p6)%>%
  filter(stems!=0) %>% 
  filter(Year!="NA")
SD2016<-read.csv("~/Dropbox/Ghost Fire/Data/GhostFire2016_Data/StemDensity/GhostFire_SpringStemD_2016_v2.csv")%>%
  select(-Format.ID) %>% 
  group_by(Year, Burn, Watershed, Block, spnum, Species) %>% 
  gather(Plot, stems, p1:p6)%>%
  filter(stems!=0)%>% 
  filter(Year!="NA")
SD2017<-read.csv("~/Dropbox/Ghost Fire/Data/GhostFire2017_Data/StemDensity/GhostFire_SpringStemD_2017.csv")%>%
  select(-Format.ID) %>% 
  group_by(Year, Burn, Watershed, Block, spnum, Species) %>% 
  gather(Plot, stems, p1:p6)%>%
  filter(stems!=0)%>% 
  filter(Year!="NA")
SD2018<-read.csv("~/Dropbox/Ghost Fire/Data/GhostFire2018_Data/StemDensity/GhostFire_SpringStemD_2018.csv")%>%
  select(-Format.ID) %>% 
  group_by(Year, Burn, Watershed, Block, spnum, Species) %>% 
  gather(Plot, stems, p1:p6)%>%
  filter(stems!=0)%>% 
...
StemDensity_AllYears<-bind_rows(SD2014, SD2015, SD2016, SD2017, SD2018)
# bring in sp list
SpList<-read.csv("~/Dropbox/Ghost Fire/Data/GhostFire_Konza_spplist.csv")
...
#SK checked 2014-2018 and found multiple errors (>15)
#SK cleaned sp comp and sp list through 2018 on May 21, 2019
#write.csv(StemDensity_AllYears2, "StemDensity_2014-2018.csv", row.names = F)
